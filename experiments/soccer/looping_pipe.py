...
def pandas_query(input_pkl_path: InputBinaryFile(), output_pkl_path: OutputBinaryFile(), transform_code: str = "df = df"):
    import numpy as np
    import pandas as pd
...
pandas_query_op = create_component_from_func(func=pandas_query, base_image='matejcvut/kubeflow-pod:0')
...
def looping_pipe(drive_file_id: str = '1oiudIrYHaxjW6sEVh_GH7YlIVT_Xi0Od',
    ...
    pandas_query_task = pandas_query_op(names2ids_task.output, transform_code = "df = df[df['Sea'] == 2000]")


    arr = np.array([2000, 2009, 2010, 2001, 2002, 2003, 2004, 2005, 2011, 2012, 2013, 2006, 2007, 2008, 2014, 2015, 2016])
